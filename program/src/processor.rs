...
use {
    crate::state::{get_config_keys_slice, ConfigKeys, PodConfigKey},
    solana_program::{
...
pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], input: &[u8]) -> ProgramResult {
...

    {
        ...
        let current_data_keys = get_config_keys_slice(&data).map_err(|err| {
            msg!("Unable to deserialize config account: {}", err);
            err
        })?;

        let current_signer_keys: Vec<Pubkey> = current_data_keys
            .iter()
            .filter_map(|PodConfigKey(key, is_signer)| {
                if bool::from(is_signer) {
                    Some(*key)
                } else {
                    None
                }
